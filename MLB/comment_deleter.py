...
def main():
    ...
    try:
        with conn:
            ...
            curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"mlb_messages\";")
            
            numtables = int(curr.fetchone()[0])

            if not numtables:
                logger.info("mlb_messages table missing. Creating...")
                curr.execute("CREATE TABLE mlb_messages (message_id TEXT PRIMARY KEY, message_author TEXT NOT NULL, was_successful BOOLEAN NOT NULL CHECK (was_successful IN (0,1)), timestamp INTEGER NOT NULL);")
                curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"mlb_messages\";")
                numtables = int(curr.fetchone()[0])
                if numtables:
                    logger.info("mlb_messages table created!")
                else:
                    raise Exception("Error creating table!")

            curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"mlb_reruns\";")
            
            numtables = int(curr.fetchone()[0])

            if not numtables:
                logger.info("mlb_reruns table missing. Creating...")
                curr.execute("CREATE TABLE mlb_reruns (message_id TEXT PRIMARY KEY, message_author TEXT NOT NULL, was_successful BOOLEAN NOT NULL CHECK (was_successful IN (0,1)), timestamp INTEGER NOT NULL);")
                curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"mlb_reruns\";")
                numtables = int(curr.fetchone()[0])
                if numtables:
                    logger.info("mlb_reruns table created!")
                else:
                    raise Exception("Error creating table!")
    finally:
        conn.close()
    ...
    if manual_message:
        ...
        if message.author and not message.author.name.lower() in mlb.blocked_users:
            ...
            elif re.search(r"!\bmlbcompare\b", message.body, re.IGNORECASE):
...
def parse_message(message, reddit):
    ...
    try:
        with conn:
            curr = conn.cursor()
            curr.execute("SELECT 1 FROM mlb_messages WHERE message_id = ?;", (message.id, ))
            if not curr.fetchone():
                ...
                try:
                    ...
                    curr.execute("SELECT reply_comment_id, reply_author, subreddit FROM mlb WHERE was_deleted = 0 AND reply_id = ?;", (comment_id, ))
                    ...
                    while(True):
                        try:
                            curr.execute("UPDATE mlb SET was_deleted = 1 WHERE reply_id = ?;", (comment_id, ))
                            break
                        except Exception:
                            failed_counter += 1
                            if failed_counter > mlb.max_request_retries:
                                raise

                        delay_step = 10
                        logger.info("#" + str(threading.get_ident()) + "#   " + "Retrying in " + str(mlb.retry_failure_delay) + " seconds to allow db to chill")
                        time_to_wait = int(math.ceil(float(mlb.retry_failure_delay)/float(delay_step)))
                        for i in range(mlb.retry_failure_delay, 0, -time_to_wait):
                ...
                while(True):
                    try:
                        curr.execute("INSERT INTO mlb_messages VALUES (?,?,?,?);", (message.id, message.author.name, was_successful, round(time.time() * 1000)))
                        break
                    except Exception:
                        failed_counter += 1
                        if failed_counter > mlb.max_request_retries:
...
def re_run_message(message, reddit, debug_mode):
    ...
    try:
        with conn:
            curr = conn.cursor()
            curr.execute("SELECT 1 FROM mlb_reruns WHERE message_id = ?;", (message.id, ))
            if not curr.fetchone():
                reddit_message = None
                was_successful = 0
                comment_id = message.body.strip().lower()
                ...
                try:
                    ...
                    try:
                        curr.execute("SELECT reply_comment_id, reply_author, subreddit, original_comment, was_successful FROM mlb WHERE was_deleted = 0 AND reply_id = ?;", (comment_id, ))
                ...
                while(True):
                    try:
                        curr.execute("INSERT INTO mlb_reruns VALUES (?,?,?,?);", (message.id, message.author.name, was_successful, round(time.time() * 1000)))
                        break
                    except Exception:
                        failed_counter += 1
                        if failed_counter > mlb.max_request_retries:
...
def parse_comment(reddit, comment_id, reply_comment_id, original_comment, is_existing_comment, message, debug_mode, curr):
    ...
    if is_message:
        ...
        if re.search(r"!\bmlbcompare\b", message.body, re.IGNORECASE):
        ...
        if not comment.archived:
            if re.search(r"!\bmlbcompare\b", comment.body, re.IGNORECASE):
                comment_obj = None
                if reply_comment_id:
