...
def main():
    ...
    try:
        with conn:
            ...
            curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"nfl_messages\";")
            
            numtables = int(curr.fetchone()[0])

            if not numtables:
                logger.info("nfl_messages table missing. Creating...")
                curr.execute("CREATE TABLE nfl_messages (message_id TEXT PRIMARY KEY, message_author TEXT NOT NULL, was_successful BOOLEAN NOT NULL CHECK (was_successful IN (0,1)), timestamp INTEGER NOT NULL);")
                curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"nfl_messages\";")
                numtables = int(curr.fetchone()[0])
                if numtables:
                    logger.info("nfl_messages table created!")
                else:
                    raise Exception("Error creating table!")

            curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"nfl_reruns\";")
            
            numtables = int(curr.fetchone()[0])

            if not numtables:
                logger.info("nfl_reruns table missing. Creating...")
                curr.execute("CREATE TABLE nfl_reruns (message_id TEXT PRIMARY KEY, message_author TEXT NOT NULL, was_successful BOOLEAN NOT NULL CHECK (was_successful IN (0,1)), timestamp INTEGER NOT NULL);")
                curr.execute("SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" AND name=\"nfl_reruns\";")
                numtables = int(curr.fetchone()[0])
                if numtables:
                    logger.info("nfl_reruns table created!")
                else:
                    raise Exception("Error creating table!")
    finally:
        conn.close()
        ...
        if manual_message:
            ...
            if message.author and not message.author.name.lower() in nfl.blocked_users:
                if message.subject.strip().lower() == "delete":
                    ...
                    nfl.parse_input(gateway, message, True, False)
            return
                ...
                if isinstance(message, Message):
                    if message.author and not message.author.name.lower() in nfl.blocked_users:
                        if message.subject.strip().lower() == "delete":
                            ...
                            logger.info("FOUND COMPARE MESSAGE " + str(message.id))
                            executor.submit(nfl.parse_input, gateway, message, False, False)
...
def parse_message(message, reddit):
    ...
    try:
        with conn:
            curr = conn.cursor()
            curr.execute("SELECT 1 FROM nfl_messages WHERE message_id = ?;", (message.id, ))
            if not curr.fetchone():
                ...
                try:
                    ...
                    curr.execute("SELECT reply_comment_id, reply_author, subreddit FROM nfl WHERE was_deleted = 0 AND reply_id = ?;", (comment_id, ))
                    ...
                    while(True):
                        try:
                            curr.execute("UPDATE nfl SET was_deleted = 1 WHERE reply_id = ?;", (comment_id, ))
                            break
                        except Exception:
                            failed_counter += 1
                            if failed_counter > nfl.max_request_retries:
                                raise

                        delay_step = 10
                        logger.info("#" + str(threading.get_ident()) + "#   " + "Retrying in " + str(nfl.retry_failure_delay) + " seconds to allow db to chill")
                        time_to_wait = int(math.ceil(float(nfl.retry_failure_delay)/float(delay_step)))
                        for i in range(nfl.retry_failure_delay, 0, -time_to_wait):
                ...
                while(True):
                    try:
                        curr.execute("INSERT INTO nfl_messages VALUES (?,?,?,?);", (message.id, message.author.name, was_successful, round(time.time() * 1000)))
                        break
                    except Exception:
                        failed_counter += 1
                        if failed_counter > nfl.max_request_retries:
...
def re_run_message(message, reddit, debug_mode):
    ...
    try:
        with conn:
            curr = conn.cursor()
            curr.execute("SELECT 1 FROM nfl_reruns WHERE message_id = ?;", (message.id, ))
            if not curr.fetchone():
                reddit_message = None
                was_successful = 0
                comment_id = message.body.strip().lower()
                ...
                try:
                    ...
                    try:
                        curr.execute("SELECT reply_comment_id, reply_author, subreddit, original_comment, was_successful FROM nfl WHERE was_deleted = 0 AND reply_id = ?;", (comment_id, ))
                ...
                while(True):
                    try:
                        curr.execute("INSERT INTO nfl_reruns VALUES (?,?,?,?);", (message.id, message.author.name, was_successful, round(time.time() * 1000)))
                        break
                    except Exception:
                        failed_counter += 1
                        if failed_counter > nfl.max_request_retries:
    ...
    if is_message:
        ...
        if re.search(r"!\bnflcompare(?:bot)?\b", message.body, re.IGNORECASE):
            ...
            try:
                ...
                logger.error("#" + str(threading.get_ident()) + "#   " + traceback.format_exc())
            nfl.parse_input(gateway, message, debug_mode, False, curr)
            was_successful = 1
        else:
        ...
        if not comment.archived:
            if re.search(r"!\bnflcompare(?:bot)?\b", comment.body, re.IGNORECASE):
                ...
                try:
                    ...
                    logger.error("#" + str(threading.get_ident()) + "#   " + traceback.format_exc())
                nfl.parse_input(gateway, comment, debug_mode, comment.subreddit.display_name in nfl.approved_subreddits, curr, comment_obj)
                was_successful = 1
            else:
