        ...
        if manual_message:
            ...
            if message.author and not message.author.name.lower() in nhl.blocked_users:
                if message.subject.strip().lower() == "delete":
                    ...
                    parse_message(gateway, message, reddit)
                    ...
                    nhl.parse_input(gateway, message, True, False)
            return
                ...
                if isinstance(message, Message):
                    if message.author and not message.author.name.lower() in nhl.blocked_users:
                        if message.subject.strip().lower() == "delete":
                            ...
                            logger.info("FOUND COMPARE MESSAGE " + str(message.id))
                            executor.submit(nhl.parse_input, gateway, message, False, False)
    finally:
    ...
    if is_message:
        ...
        if re.search(r"!\bnhlcompare(?:bot)?\b", message.body, re.IGNORECASE):
            ...
            try:
                ...
                logger.error("#" + str(threading.get_ident()) + "#   " + traceback.format_exc())
            nhl.parse_input(gateway, message, debug_mode, False, curr)
            was_successful = 1
        else:
        ...
        if not comment.archived:
            if re.search(r"!\bnhlcompare(?:bot)?\b", comment.body, re.IGNORECASE):
                ...
                try:
                    ...
                    logger.error("#" + str(threading.get_ident()) + "#   " + traceback.format_exc())
                nhl.parse_input(gateway, comment, debug_mode, comment.subreddit.display_name in nhl.approved_subreddits, curr, comment_obj)
                was_successful = 1
            else:
